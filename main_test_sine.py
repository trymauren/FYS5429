...
def model_performance_sine(rnn, num_tests, test_length, seed_length = 10, seq_length = 100):
    ...
    for sine in X_val:
        ...
        predict = rnn.predict(X_seed, time_steps_to_generate =
                                      time_steps_to_predict)
        accuracies.append(error(predict,sine[random_start + seed_length:random_start + test_length]))
        ax.plot(sine[random_start + seed_length:random_start + test_length], label = "True")
...
for learning_rate_curr in learning_rates:
    ...
    for optimiser, i in zip(optimisers, range(len(optimisers))):
        ...
        performance = model_performance_sine(rnn, 30, 10)
